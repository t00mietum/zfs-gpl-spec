...
| 8 | 2026-07-22 | ZPL znode and system-attribute layout plus directory ZAP entry encoding: the filesystem master node and ZPL versions, the legacy fixed znode record (field offsets, bonus-tail symlink/scanstamp), the SA mechanism (registry and layout tables, header, attribute placement, spill), the attribute set (timestamps, mode/size/parent/links, flags, FUID ids, device ids, project id), directory-entry value encoding (object number and type bits) and name matching, the unlinked set, extended-attribute storage, and ACL storage containers - needed to walk directories and read file metadata from a filesystem object set | spec'd -> `specs/format/08-zpl-znode-sa-dirent.md` @ `aa97c46`
